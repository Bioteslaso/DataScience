...
% What is done by these instructions? Centering the points, putting the
...
w1=2*pi*uc/sz;
w2=2*pi*vc/sz;
wavelength = 2*pi*(1/(sqrt(w1^2+w2^2)));  % Replace by correct expression 
amplitude = max(max(abs(Fhat(:)))/(sz));   % Replace by correct expression
